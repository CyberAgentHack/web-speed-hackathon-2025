...
async function main() {
	...
	try {
		...
		{
			const data: (typeof schema.channel.$inferInsert)[] =
				CHANNEL_NAME_LIST.map(({ id, name }) => ({
		...
		for (const reference of [
			...seriesList.map(
				(s) => ({ id: s.id, type: "series", series: s }) as const,
			),
			...episodeList.map(
				(e) => ({ id: e.id, type: "episode", episode: e }) as const,
			),
			...programList.map(
				(p) => ({ id: p.id, type: "program", program: p }) as const,
			),
						...
						switch (reference.type) {
							...
							case "episode": {
								const series = seriesList.find(
									(s) => s.id === reference.episode.seriesId,
								);
								const relatedEpisodes = episodeList.filter(
									(e) => e.seriesId === series?.id,
								);
								return relatedEpisodes.every((r) => r.id !== target.id);
							}
							case "program": {
								const targetEpisode = episodeList.find(
									(s) => s.id === reference.program.episodeId,
								);
								const series = seriesList.find(
									(s) => s.id === targetEpisode?.seriesId,
								);
								const relatedEpisodes = episodeList.filter(
									(e) => e.seriesId === series?.id,
								);
