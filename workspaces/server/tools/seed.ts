...
async function main() {
  ...
  const imagePaths = files.map((file) =>
    path.join("/", path.relative(rootDir, file))
  );
  ...
  try {
    ...
    {
      const data: (typeof schema.channel.$inferInsert)[] = CHANNEL_NAME_LIST
        .map(({ id, name }) => ({
          id: faker.string.uuid(),
          logoUrl: `/public/logos/${id}.svg`,
          name,
        }));
      const result = await database.insert(schema.channel).values(data)
        .returning();
    ...
    {
      const data: (typeof schema.series.$inferInsert)[] = Array.from({
        length: 30,
      }, () => ({
        description: faker.lorem.paragraph({ max: 200, min: 100 }).replace(
          /\s/g,
          "",
        ).replace(/\./g, "。"),
        id: faker.string.uuid(),
        thumbnailUrl: `${
          faker.helpers.arrayElement(imagePaths)
        }?version=${faker.string.nanoid()}`,
        title: faker.helpers.arrayElement(seriesTitleList),
      }));
      const result = await database.insert(schema.series).values(data)
        .returning();
    ...
    for (const series of seriesList) {
      const data: (typeof schema.episode.$inferInsert)[] = Array.from(
        ...
        (_, idx) => ({
          ...
          title: `第${String(idx + 1)}話 ${
            faker.helpers.arrayElement(episodeTitleList)
          }`,
          premium: idx % 5 === 0,
        }),
      );
      const result = await database.insert(schema.episode).values(data)
        .returning();
    ...
    const episodeListGroupedByStreamId = Object.values(
      Object.groupBy(episodeList, (episode) => episode.streamId),
    );
    for (const channel of channelList) {
      let remainingMinutes = 24 * 60;
      let startAt = new Date().setHours(0, 0, 0, 0);

      while (remainingMinutes > 0) {
        const durationCandidate = channel.name === "ニュース"
          ? 5
          : faker.number.int({ max: 120, min: 15, multipleOf: 15 });
    ...
    for (
      const reference of [
        ...seriesList.map((s) =>
          ({ id: s.id, type: "series", series: s }) as const
        ),
        ...episodeList.map((e) =>
          ({ id: e.id, type: "episode", episode: e }) as const
        ),
        ...programList.map((p) =>
          ({ id: p.id, type: "program", program: p }) as const
        ),
            ...
            switch (reference.type) {
              ...
              case "episode": {
                const series = seriesList.find((s) =>
                  s.id === reference.episode.seriesId
                );
                const relatedEpisodes = episodeList.filter((e) =>
                  e.seriesId === series?.id
                );
                return relatedEpisodes.every((r) => r.id !== target.id);
              }
              case "program": {
                const targetEpisode = episodeList.find((s) =>
                  s.id === reference.program.episodeId
                );
                const series = seriesList.find((s) =>
                  s.id === targetEpisode?.seriesId
                );
                const relatedEpisodes = episodeList.filter((e) =>
                  e.seriesId === series?.id
                );
      ...
      const moduleList = await database
        ...
        .values(
          Array.from({ length: 20 }, (_, moduleOrder) => {
            const moduleType =
              reference.id === "entrance" && moduleOrder % 4 === 0
                ? "jumbotron"
                : "carousel";
            ...
            return {
              ...
              title: moduleType === "jumbotron"
                ? ""
                : `『${
                  faker.helpers.arrayElement(seriesTitleList)
                }』を見ているあなたにオススメ`,
              type: moduleType,
            };
          }),
      ...
      for (const module of moduleList) {
        if (module.type === "jumbotron") {
          ...
          await database.insert(schema.recommendedItem).values(
            Array.from(
              { length: faker.number.int({ max: 20, min: 15 }) },
              (_, itemOrder) => ({
                episodeId: null,
                id: faker.string.uuid(),
                moduleId: module.id,
