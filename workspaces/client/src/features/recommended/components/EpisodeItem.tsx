...
const areEqual = (prevProps: { episode: { id: string } }, nextProps: { episode: { id: string } }) => {
  return prevProps.episode.id === nextProps.episode.id;  // `id` が同じなら再レンダリングを避ける
};
            ...
            <>
              <Flipped stagger flipId={isTransitioning ? `episode-${episode.id}` : 0}>
                <div className="relative overflow-hidden rounded-[8px] border-[2px] border-solid border-[#FFFFFF1F] before:absolute before:inset-x-0 before:bottom-0 before:block before:h-[64px] before:bg-gradient-to-t before:from-[#212121] before:to-transparent before:content-['']">
                  <img alt="" className="h-auto w-full" src={changeImageExtension(episode.thumbnailUrl)} loading='lazy' decoding='async'/>
...
// カスタム比較関数を渡してメモ化
export default memo(EpisodeItem, areEqual);
