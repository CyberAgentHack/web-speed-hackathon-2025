...
export const Layout = ({ children }: Props) => {
	...
	const isLoading =
		...
		(navigation.location.state as { loading?: string } | null)?.["loading"] !==
			"none";
	...
	const [shouldHeaderBeTransparent, setShouldHeaderBeTransparent] =
		useState(false);

	useEffect(() => {
		const handleScroll = () => {
			const isOverThreshold = window.scrollY > 80;

			if (
				(isOverThreshold && !shouldHeaderBeTransparent) ||
				(!isOverThreshold && shouldHeaderBeTransparent)
			) {
				setShouldHeaderBeTransparent(isOverThreshold);
			}
		};
