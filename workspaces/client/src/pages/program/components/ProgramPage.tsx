...
export const ProgramPage = () => {
	...
	// invariant(program);
	const { program, modules, timetable } = useLoaderData();
	...
	const isArchivedRef = useRef(
		DateTime.fromISO(program.endAt) <= DateTime.now(),
	);
	const isBroadcastStarted =
		DateTime.fromISO(program.startAt) <= DateTime.now();
	useEffect(() => {
				...
				<div className="mb-[24px]">
					<div className="text-[16px] text-[#ffffff]">
						<Ellipsis
							...
							reflowOnResize
							maxLine={1}
							text={program.episode.series.title}
							visibleLine={1}
						/>
					</div>
