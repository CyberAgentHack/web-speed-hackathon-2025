...
import faRegularIcons from '@iconify/json/json/fa-regular.json';
  ...
  await initUnocssRuntime({
    defaults: defineConfig({
      ...
      presets: [
        presetWind3(),
        presetIcons({
          scale: 1.2, // アイコンサイズを小さくして軽量化
          collections: {
            bi: () => Promise.resolve(biIcons as IconifyJSON),
            bx: () => Promise.resolve(bxIcons as IconifyJSON),
            'fa-regular': () => Promise.resolve(faRegularIcons as IconifyJSON),
...
init().catch((err: unknown) => {
  console.error('Unocss Runtime Error:', err);
});
